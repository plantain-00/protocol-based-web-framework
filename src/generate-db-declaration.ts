...
export default (typeDeclarations: TypeDeclaration[]): { path: string, content: string }[] => {
  ...
  const content = `import { RowFilterOptions, RowSelectOneOptions, RowSelectOptions, getKeys } from "${process.env.DB_DECLARATION_LIB_PATH || 'protocol-based-web-framework'}"
